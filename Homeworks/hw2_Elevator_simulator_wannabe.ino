...
bool isMoving = false;  // Starea curenta a liftului
...
void setup() {
  ...
  digitalWrite(floor0LED, HIGH);     // Initializare stare curenta
}

void loop() {
  // Verificare butoane apasate
...
void moveElevator(int targetFloor) {
  if (!isMoving) {
    ...
    if((currentFloor == 0 && targetFloor == 1) || (currentFloor == 1 && targetFloor == 2) || (currentFloor == 1 && targetFloor == 0) || (currentFloor == 2 && targetFloor == 1)) {
      playFloorSound2();  // Reda sunetul la trecerea prin etaj
      ...
      playFloorSound1();  // Reda sunetul la trecerea prin etaj
      digitalWrite(elevatorMovingLED, LOW); 
      delay(500);
      digitalWrite(elevatorMovingLED, HIGH);
      delay(500); 
    ...
    // Actualizeaza etajul curent si LED-urile etajelor
    currentFloor = targetFloor;
    updateFloorLEDs();
    stopElevator();  // Opreste liftul după sosirea la destinatie
  }
   playFloorSound();  // Reda sunetul la ajungerea liftului
}
...
void playFloorSound() {
  tone(buzzerPin, 1000, 200);  // Genereaza un sunet de 1000 Hz pentru 200 de milisecunde
}
void playFloorSound1()
{
  tone(buzzerPin, 500, 4000);  // Genereaza un sunet de 500 Hz pentru 4000 de milisecunde
}
void playFloorSound2()
{
  tone(buzzerPin, 500, 1000);  // Genereaza un sunet de 1000 Hz pentru 1000 de milisecunde
}
